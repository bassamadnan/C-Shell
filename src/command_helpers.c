...
void print_bg(){
    // printf("%d bgc \n", bgc);
    for(int i=0; i<=bgc; i++){
        // printf("%s cmd is over: %d \n", bgarr[i].command, bgarr[i].isover);
        if(bgarr[i].isover && !bgarr[i].printed){
            if(bgarr[i].normal){
                printf("%s with pid: %d exited normally \n", bgarr[i-1].command, bgarr[i-1].pid);
            }else{
                printf("%s with pid: %d exited abnormally \n", bgarr[i-1].command, bgarr[i-1].pid);
            }
            // double ans = difftime(bgarr[i].end, bgarr[i].start);
            // printf("\t\t #After %2d seconds \n", (int) ans);
